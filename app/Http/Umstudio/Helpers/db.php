...
if (!function_exists('db_get_primary_key'))
{
	...
	{
		$register = DB::select(sprintf('SHOW KEYS FROM %s%s WHERE Key_name = "PRIMARY"', env('DB_PREFIX'), $table_name));
		$register = $register[0];
...
if (!function_exists('db_get_name'))
{
	...
	{
		$register = DB::select(sprintf('SELECT `name` FROM `%s%s` WHERE `id` = "%s";', env('DB_PREFIX'), $table_name, $id));
		if (empty($register))
		{
			return '';
		}
